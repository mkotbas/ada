...
import { clear, make, appendIconText, parseHtmlFragment, installFormFieldIdentityObserver } from '../core/dom';
...
import { canAccessAdminModule, canAccessAdminShell, getAccessibleAdminModuleIds, getCurrentUserRole, type AppRole } from '../core/permissions';
...
function getAccessibleModules(userRole: AppRole): ModuleItem[] {
  const allowedIds = new Set(getAccessibleAdminModuleIds(userRole));
  return MODULES.filter(module => allowedIds.has(module.id as never));
}
...
function getInitialModuleId(userRole: AppRole): string {
...
async function initializeAdminPanel(): Promise<void> {
  ...
  const userRole = isLoggedIn ? getCurrentUserRole(pb.authStore.model as Record<string, unknown> | null) : null;
...
function renderModuleMenu(userRole: AppRole): void {
  const menu = document.getElementById('module-menu');
  if (!menu) return;
  clear(menu);
...
async function loadModule(moduleId: string): Promise<void> {
  const currentRole = getCurrentUserRole(pb.authStore.model as Record<string, unknown> | null);
  if (!canAccessAdminModule(currentRole, moduleId)) {
    showAccessDenied();
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  installFormFieldIdentityObserver();
