...
export const MAX_MANUAL_AUDIT_COUNT = 20;
...
>): MonthlyAuditConsistency {
  ...
  const totalPlannedFromNow = Object.entries(state.planByDay).reduce((total, [dayKey, plannedCount]) => {
    const day = Number(dayKey);
    const completedForDay = state.doneByDayRaw[day] ?? 0;
    return total + Math.max(0, plannedCount - completedForDay);
  }, 0);
...
function isValidManualAuditDateKey(key: string): boolean {
...
function normalizeManualAuditCount(value: unknown): number {
  const flooredValue = Math.floor(normalizeNumber(value, 0));
  return Math.min(MAX_MANUAL_AUDIT_COUNT, Math.max(0, flooredValue));
}
...
export function parseManualAuditMap(rawValue: unknown): ManualAuditMap {
  ...
  return Object.entries(parsed).reduce<ManualAuditMap>((acc, [key, value]) => {
    if (!isValidManualAuditDateKey(key)) return acc;

    const numericValue = normalizeManualAuditCount(value);
    if (numericValue > 0) {
...
export function expandManualCountsToReports(manualAuditData: ManualAuditMap, year: number, month: number): DayReport[] {
  ...
  Object.entries(manualAuditData).forEach(([key, count]) => {
    ...
    const normalizedCount = normalizeManualAuditCount(count);
...
export async function loadMonthlyAuditState(pb: PocketBase, options: LoadMonthlyAuditStateOptions): Promise<MonthlyAuditState> {
  ...
  const [settings, reports, reverts] = await Promise.all([
    ...
    fetchAllRecords<ReportLike>(pb.collection('denetim_raporlari'), {
      filter: reportFilterParts.join(' && '),
      expand: 'bayi',
      sort: '-updated',
    }),
    fetchAllRecords<RevertLike>(pb.collection('denetim_geri_alinanlar'), {
