...
export function buildExactDailyDistribution(
  ...
  if (normalizedTarget >= minimumCapacity) {
    ...
    const evenBonus = Math.floor((normalizedTarget - minimumCapacity) / orderedDays.length);
    const bonusRemainder = (normalizedTarget - minimumCapacity) % orderedDays.length;

    if (evenBonus > 0) {
      orderedDays.forEach((day) => {
        distribution[day] = (distribution[day] ?? 0) + evenBonus;
      });
    }

    for (let index = 0; index < bonusRemainder; index += 1) {
...
export function calculateCurrentMonthPlan(
  ...
  const normalizedMin = Math.max(0, Math.floor(minDaily) || 0);
  const completedTodayRaw = doneByDayRaw[todayDay] ?? 0;

  const planByDay: DailyPlanMap = {};
  const futureActiveDays = activeDays.filter(day => day > todayDay);

  const todayFloorNeed = activeToday
    ? Math.max(0, normalizedMin - completedTodayRaw)
    : 0;
  const todayAdditionalNeed = Math.min(totalRemaining, todayFloorNeed);
  ...
  const todayRemainingNeed = todayAdditionalNeed + undistributedAfterFuture;

  if (activeToday && (completedTodayRaw > 0 || todayRemainingNeed > 0)) {
    planByDay[todayDay] = completedTodayRaw + todayRemainingNeed;
  } else if (todayRemainingNeed > 0) {
    const fallbackDay = futureActiveDays[futureActiveDays.length - 1] ?? activeDays[activeDays.length - 1] ?? todayDay;
    futurePlan[fallbackDay] = (futurePlan[fallbackDay] ?? 0) + todayRemainingNeed;
  }
  ...
  return {
    ...
    diagnostics: {
      ...
      todayAdditionalNeed: todayRemainingNeed,
      futureTotalPlanned: sumPlan(futurePlan),
      totalPlannedFromNow: todayRemainingNeed + sumPlan(futurePlan),
      activeToday,
