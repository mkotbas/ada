...
function isPocketBaseClientErrorWithField(error: unknown, statusCode: number, fieldName: string): boolean {
  if (typeof error !== 'object' || error === null || !('status' in error) || Number(error.status) !== statusCode) {
    return false;
  }

  const responseData = 'response' in error ? error.response : null;
  if (typeof responseData !== 'object' || responseData === null || !('data' in responseData)) {
    return false;
  }

  const fieldErrors = responseData.data;
  return typeof fieldErrors === 'object' && fieldErrors !== null && fieldName in fieldErrors;
}

async function findAuditHistoryRecordId(uniqueKeyGun: string): Promise<string | null> {
  const records = await pb.collection('denetim_gecmisi').getList<PocketBaseRecord>(1, 1, {
    filter: `uniqueKeyGun = "${uniqueKeyGun}"`,
  });

  const existingRecord = records.items[0];
  return existingRecord ? String(existingRecord.id ?? '') : null;
}
...
export async function saveAuditHistory(storeId: string, finalizedAtIso: string): Promise<void> {
  ...
  const uniqueKeyGun = String(payload['uniqueKeyGun'] ?? '').trim();
  if (!uniqueKeyGun) return;

  const existingRecordId = await findAuditHistoryRecordId(uniqueKeyGun);
  if (existingRecordId) return;

  try {
    await pb.collection('denetim_gecmisi').create(payload);
  } catch (error) {
    if (isPocketBaseClientErrorWithField(error, 400, 'uniqueKeyGun')) {
      return;
    }
...
export async function saveFormState(reportData: ReportData, isFinalizing = false): Promise<string | null> {
  ...
  if (isFinalizing) {
    try {
      ...
      const undoneRecords = await pb.collection('denetim_geri_alinanlar').getList<PocketBaseRecord>(1, 1, {
        filter: `yil_ay="${monthKey}" && bayi="${storeRecord.id}"`,
      });
      const undoneRecord = undoneRecords.items[0];
      if (undoneRecord?.id) {
