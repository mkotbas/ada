...
async function loadMonthlyAuditData(): Promise<void> {
  ...
  try {
    const revertedRecords = await pb.collection('denetim_geri_alinanlar').getFullList<PocketBaseRecord>({
...
async function loadQuestionSettings(): Promise<void> {
  const ayarlarRecords = await pb.collection('ayarlar').getFullList<PocketBaseRecord>();
...
async function loadStoreList(): Promise<void> {
  const records = await pb.collection('bayiler').getFullList<Store>({ sort: 'bayiAdi' });
...
export async function saveFormState(reportData: ReportData, isFinalizing = false): Promise<void> {
  ...
  if (isFinalizing) {
    dataToSave['denetimTamamlanmaTarihi'] = getBusinessUtcIsoNow();
