...
function handleFormClick(e) {
  ...
  if (btn) {
    ...
    if (
      btn.classList.contains("delete-bar") ||
      btn.classList.contains("delete-item-btn")
    ) {
...
function createSectionLead(text) {
...
input-area",
  ...
  return (
    ...
    (q.staticItems ?? []).forEach((item) =>
      container.append(createStaticItem(item)),
    ),
...
function createStaticItem(text) {
  ...
  return (
    ...
    div.querySelector("button")?.replaceChildren(
      make("i", {
...
function buildEmailListItemsFromContainer(container) {
  return Array.from(container.children)
    ...
    .filter((node) => !node.classList.contains("is-deleting"))
...
form-select",
  ...
  const qtyInput = make("input", {
    ...
    attrs: { placeholder: "Adet", min: "1", value: "1" },
  });
...
ri\u015F / Malzeme Ekle",
  ...
  (standardView.id = `standard-view-container-${q.id}`),
    (q.staticItems ?? []).forEach((item) =>
      standardView.append(createStaticItem(item)),
    );
...
>{PUAN_TABLOSU}";
  ...
  await saveFormState(reportData, !0),
