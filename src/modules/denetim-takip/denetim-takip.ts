...
import { appendIconText, escapeHtml, setSelectPlaceholder } from '../../core/dom';
import { errorService } from '../../core/error';
import { getBusinessDateParts, getBusinessYearMonthKey, getBusinessYearUtcRange, toBusinessZonedDateTime } from '../../core/temporal';
...
interface FrequencyStatusSummary {
  tamamlandi: number;
  asildi: number;
  devamEdiyor: number;
  baslanmadi: number;
...
interface StoreFrequencyMetric {
  store: StoreRecord;
  target: number | null;
  completed: number;
  remaining: number | null;
  percentage: number | null;
  statusKey: 'tamamlandi' | 'asildi' | 'devam-ediyor' | 'baslanmadi' | 'tanimsiz';
  statusLabel: string;
}
...
let allAuditHistoryMaster: AuditHistoryRecord[] = [];
...
let frequencyDataMap = new Map<string, number>();
let currentYearAuditCountMap = new Map<string, number>();

let currentGlobalFilteredStores: StoreRecord[] = [];
let localCityFilterValue = 'Tümü';
let currentViewMode: 'monthly' | 'yearly' = 'monthly';
let frequencyStatusFilterValue = 'tum';
let frequencyListViewMode: 'preview' | 'all' | 'issues' = 'preview';
...
function isArchivedStateValue(value: unknown): boolean {
  return value === true || value === 'true' || value === 1 || value === '1';
}

function isStoreArchived(store: StoreRecord): boolean {
  const storeId = String(store.id ?? '').trim();
  const storeCode = normalizeDealerCode(store.bayiKodu);
  return archivedStoreIdSet.has(storeId)
    || archivedStoreCodeSet.has(storeCode)
    || isArchivedStateValue((archivedStoreState as Record<string, unknown>)[storeId])
    || isArchivedStateValue((archivedStoreState as Record<string, unknown>)[storeCode]);
}

function isStoreVisibleForFrequency(store: StoreRecord): boolean {
  if (isStoreArchived(store)) return false;

  const storeId = String(store.id ?? '').trim();
  const storeCode = normalizeDealerCode(store.bayiKodu);

  return allStores.some(activeStore => {
    const activeStoreId = String(activeStore.id ?? '').trim();
    const activeStoreCode = normalizeDealerCode(activeStore.bayiKodu);
    return (storeId && activeStoreId === storeId) || (!!storeCode && activeStoreCode === storeCode);
  });
}

function getActiveFrequencyStores(stores: StoreRecord[]): StoreRecord[] {
  return stores.filter(isStoreVisibleForFrequency);
}

function getUniqueAuditDayKey(record: AuditHistoryRecord, code: string): string | null {
  const uniqueKeyGun = typeof record.uniqueKeyGun === 'string' ? record.uniqueKeyGun.trim() : '';
  if (uniqueKeyGun) return `${code}__${uniqueKeyGun}`;

  if (typeof record.denetimTarihi === 'string' && record.denetimTarihi.trim()) {
    try {
      const zonedDate = toBusinessZonedDateTime(record.denetimTarihi);
      const month = String(zonedDate.month).padStart(2, '0');
      const day = String(zonedDate.day).padStart(2, '0');
      return `${code}__${zonedDate.year}-${month}-${day}`;
    } catch {
      // ignore invalid date and fall through to legacy fields
    }
  }

  const year = Number(record.denetimYili ?? 0);
  const month = Number(record.denetimAyi ?? 0);
  if (Number.isInteger(year) && Number.isInteger(month) && month >= 1 && month <= 12) {
    return `${code}__${year}-${String(month).padStart(2, '0')}__${record.id}`;
  }

  return null;
}

function getReportStoreCode(record: ReportRecord): string {
  if (typeof record.expand?.bayi?.bayiKodu === 'string') {
    return record.expand.bayi.bayiKodu.trim();
  }

  const storeId = String(record.bayi ?? '').trim();
  if (!storeId) return '';

  const matchedStore = allStores.find(store => String(store.id ?? '').trim() === storeId)
    ?? allStoresMaster.find(store => String(store.id ?? '').trim() === storeId);

  return normalizeDealerCode(matchedStore?.bayiKodu);
}

function getFrequencyMetric(store: StoreRecord): StoreFrequencyMetric {
  const code = normalizeDealerCode(store.bayiKodu);
  const target = frequencyDataMap.get(code) ?? null;
  const completed = currentYearAuditCountMap.get(code) ?? 0;
  ...
  if (target === null) {
    return {
      ...
      percentage: null,
      statusKey: 'tanimsiz',
      statusLabel: 'Frekans Tanımsız',
    };
  }

  const remaining = Math.max(target - completed, 0);
  const percentage = target === 0 ? (completed > 0 ? 100 : 0) : Math.min(Math.round((completed / target) * 100), 999);

  if (completed > target) {
    return { store, target, completed, remaining: 0, percentage, statusKey: 'asildi', statusLabel: 'Hedef Aşıldı' };
  }

  if (completed === target) {
    return { store, target, completed, remaining: 0, percentage, statusKey: 'tamamlandi', statusLabel: 'Tamamlandı' };
  }

  if (completed === 0) {
    return { store, target, completed, remaining, percentage, statusKey: 'baslanmadi', statusLabel: 'Başlanmadı' };
  }

  return { store, target, completed, remaining, percentage, statusKey: 'devam-ediyor', statusLabel: 'Devam Ediyor' };
}

function getFrequencySummary(metrics: StoreFrequencyMetric[]): FrequencyStatusSummary {
  return metrics.reduce<FrequencyStatusSummary>((acc, metric) => {
    if (metric.statusKey === 'tamamlandi') acc.tamamlandi += 1;
    else if (metric.statusKey === 'asildi') acc.asildi += 1;
    else if (metric.statusKey === 'devam-ediyor') acc.devamEdiyor += 1;
    else if (metric.statusKey === 'baslanmadi') acc.baslanmadi += 1;
    else acc.tanimsiz += 1;
    return acc;
  }, { tamamlandi: 0, asildi: 0, devamEdiyor: 0, baslanmadi: 0, tanimsiz: 0 });
}

function sortFrequencyMetrics(metrics: StoreFrequencyMetric[]): StoreFrequencyMetric[] {
  const getTargetSortValue = (metric: StoreFrequencyMetric): number => metric.target ?? -1;

  return [...metrics].sort((a, b) => {
    const targetCompare = getTargetSortValue(b) - getTargetSortValue(a);
    if (targetCompare !== 0) return targetCompare;

    const completedCompare = b.completed - a.completed;
    if (completedCompare !== 0) return completedCompare;

    const remainingCompare = (b.remaining ?? -1) - (a.remaining ?? -1);
    if (remainingCompare !== 0) return remainingCompare;

    return a.store.bayiAdi.localeCompare(b.store.bayiAdi, 'tr');
  });
}

async function loadFrequencyData(): Promise<void> {
  try {
    const record = await pbInstance!.collection('ayarlar').getFirstListItem(`anahtar="${FREQUENCY_STORAGE_KEY}"`).catch(() => null);
    const rows = Array.isArray(record?.['deger']) ? record['deger'] : [];
    const nextMap = new Map<string, number>();

    rows.forEach((row) => {
      const code = normalizeDealerCode(row?.['Bayi Kodu']);
      const target = normalizeFrequencyValue(row?.['Senelik Frekans']);
      if (!code || target === null) return;
      nextMap.set(code, target);
    });
...
function renderFrequencyOverview(metrics: StoreFrequencyMetric[]): void {
  const summaryRoot = document.getElementById('frequency-summary-cards');
  const tableBody = document.getElementById('frequency-progress-table-body');
  const emptyState = document.getElementById('frequency-progress-empty');
  const updatedLabel = document.getElementById('frequency-progress-year-label');
  const totalConfigured = document.getElementById('frequency-total-configured-count');

  if (!summaryRoot || !tableBody || !emptyState || !updatedLabel || !totalConfigured) return;

  const year = getBusinessDateParts().year;
  updatedLabel.textContent = `${year} yılı frekans takibi`;

  const activeMetrics = metrics.filter(metric => isStoreVisibleForFrequency(metric.store));

  const configuredCount = activeMetrics.filter(metric => metric.target !== null).length;
  totalConfigured.textContent = String(configuredCount);

  const summary = getFrequencySummary(activeMetrics);
  const summaryItems = [
    { label: 'Tamamlanan', value: summary.tamamlandi, className: 'is-complete' },
    { label: 'Hedefi Aşan', value: summary.asildi, className: 'is-over' },
    { label: 'Devam Eden', value: summary.devamEdiyor, className: 'is-progress' },
    { label: 'Başlanmayan', value: summary.baslanmadi, className: 'is-not-started' },
    { label: 'Tanımsız', value: summary.tanimsiz, className: 'is-undefined' },
  ];

  summaryRoot.replaceChildren(...summaryItems.map((item) => {
    const card = document.createElement('article');
    card.className = `frequency-mini-card ${item.className}`;

    const label = document.createElement('span');
    label.className = 'frequency-mini-card-label';
    label.textContent = item.label;

    const value = document.createElement('strong');
    value.className = 'frequency-mini-card-value';
  ...
  const wrapper = document.getElementById('frequency-table-wrapper');
  const visibleCountLabel = document.getElementById('frequency-visible-count-label');
  const filteredMetrics = activeMetrics.filter(metric => frequencyStatusFilterValue === 'tum' || metric.statusKey === frequencyStatusFilterValue);
  const scopedMetrics = frequencyListViewMode === 'issues'
    ? filteredMetrics.filter(metric => metric.statusKey !== 'tamamlandi')
    : filteredMetrics;
  const sortedMetrics = sortFrequencyMetrics(scopedMetrics);
  const visibleMetrics = frequencyListViewMode === 'preview' ? sortedMetrics.slice(0, 10) : sortedMetrics;

  tableBody.replaceChildren();

  if (wrapper) {
    wrapper.classList.toggle('is-preview', frequencyListViewMode === 'preview');
  }

  if (visibleCountLabel) {
    if (frequencyListViewMode === 'preview') {
      visibleCountLabel.textContent = `İlk ${visibleMetrics.length} / ${sortedMetrics.length} kayıt gösteriliyor`;
    } else if (frequencyListViewMode === 'issues') {
      visibleCountLabel.textContent = `${visibleMetrics.length} sorunlu bayi listeleniyor`;
    } else {
      visibleCountLabel.textContent = `${visibleMetrics.length} kayıt listeleniyor`;
    }
  }
  ...
  visibleMetrics.forEach((metric) => {
    ...
    const percentText = metric.percentage === null ? '-' : `%${metric.percentage}`;
    const remainingText = metric.remaining === null ? '-' : String(metric.remaining);
    const targetText = metric.target === null ? '-' : String(metric.target);

    row.innerHTML = `
      <td>${escapeHtml(metric.store.bayiKodu)}</td>
      <td>${escapeHtml(metric.store.bayiAdi)}</td>
      <td>${escapeHtml(metric.store.bolge ?? '-')}</td>
      <td>${targetText}</td>
      <td>${metric.completed}</td>
      <td>${remainingText}</td>
      <td>${percentText}</td>
      <td><span class="frequency-status-badge ${metric.statusKey}">${escapeHtml(metric.statusLabel)}</span></td>
    `;
...
function refreshArchivedStoreSets(): void {
  const archivedKeys = new Set(
    Object.keys(archivedStoreState)
      .map(key => String(key ?? '').trim())
      .filter(key => key && archivedStoreState[key]),
  );

  archivedStoreIdSet = new Set(
    allStoresMaster
      .map(store => String(store.id ?? '').trim())
      .filter(storeId => storeId && archivedKeys.has(storeId)),
  );

  archivedStoreCodeSet = new Set(
    allStoresMaster
      .map(store => normalizeDealerCode(store.bayiKodu))
      .filter(storeCode => storeCode && archivedKeys.has(storeCode)),
  );

  allStoresMaster.forEach(store => {
    const storeId = String(store.id ?? '').trim();
    const storeCode = normalizeDealerCode(store.bayiKodu);
    if ((storeId && archivedKeys.has(storeId)) || (storeCode && archivedKeys.has(storeCode))) {
      if (storeId) archivedStoreIdSet.add(storeId);
      if (storeCode) archivedStoreCodeSet.add(storeCode);
    }
  });
}
...
async function loadMasterData(): Promise<void> {
  try {
    ...
    allStoresMaster = await fetchAllRecords<StoreRecord>(pbInstance!.collection('bayiler'), { sort: 'bayiAdi' });
    ...
    allReportsMaster = await fetchAllRecords(pbInstance!.collection('denetim_raporlari'), {
      filter: reportDateFilter,
      expand: 'bayi',
      sort: '-updated',
    }) as ReportRecord[];
    allAuditHistoryMaster = await fetchAllRecords(pbInstance!.collection('denetim_gecmisi'), {
      filter: `durum = "tamamlandi" && denetimYili = ${businessToday.year}`,
      expand: 'bayi',
      sort: '-denetimTarihi',
    }) as AuditHistoryRecord[];
    allGeriAlinanMaster = await fetchAllRecords(pbInstance!.collection('denetim_geri_alinanlar'), {
...
async function populateUserFilterDropdown(): Promise<void> {
  try {
    allUsers = await fetchAllRecords<UserRecord>(pbInstance!.collection('users'), { sort: 'name' });
...
async function applyDataFilterAndRunDashboard(viewId: string): Promise<void> {
  ...
  const yearlyAuditedStoreMap = new Map<string, AuditedStore>();
  const nextYearAuditCountMap = new Map<string, number>();
  const countedStoreDayKeys = new Set<string>();
  const visibleStoreIdSet = new Set(allStores.map(store => String(store.id ?? '').trim()));

  filteredReports.forEach((record) => {
    const storeId = String(record.bayi ?? '').trim();
    if (!storeId || !visibleStoreIdSet.has(storeId)) return;

    const code = getReportStoreCode(record);
    if (!code || yearlyAuditedStoreMap.has(code)) return;

    const auditTimestamp = Date.parse(record.denetimTamamlanmaTarihi || record.created || '') || 0;
    yearlyAuditedStoreMap.set(code, { code, timestamp: auditTimestamp });
  });

  allAuditHistoryMaster.forEach((record) => {
    const storeId = String(record.bayi ?? '').trim();
    if (!storeId || !visibleStoreIdSet.has(storeId)) return;

    const code = typeof record.expand?.bayi?.bayiKodu === 'string' ? record.expand.bayi.bayiKodu.trim() : '';
    if (!code) return;

    const uniqueAuditDayKey = getUniqueAuditDayKey(record, code);
    if (!uniqueAuditDayKey || countedStoreDayKeys.has(uniqueAuditDayKey)) return;

    countedStoreDayKeys.add(uniqueAuditDayKey);
    nextYearAuditCountMap.set(code, (nextYearAuditCountMap.get(code) ?? 0) + 1);
  });
  currentYearAuditCountMap = nextYearAuditCountMap;
  ...
  auditedStoreCodesCurrentYear = Array.from(yearlyAuditedStoreMap.values())
...
function calculateAndDisplayDashboard(): void {
  ...
  renderFrequencyOverview(getActiveFrequencyStores(currentGlobalFilteredStores).map(getFrequencyMetric));
...
function applyAndRepopulateFilters(changedFilterId?: string): void {
  ...
  const selections = getSelectedFilterValues();
  currentGlobalFilteredStores = getStoresMatchingSelections(selections).filter(isStoreVisibleForFrequency);
...
function setupModuleEventListeners(role: string): void {
  ...
  document.getElementById('frequency-status-filter')?.addEventListener('change', (e) => {
    frequencyStatusFilterValue = (e.target as HTMLSelectElement).value;
    renderFrequencyOverview(getActiveFrequencyStores(currentGlobalFilteredStores).map(getFrequencyMetric));
  });

  document.querySelectorAll<HTMLButtonElement>('.frequency-view-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll<HTMLButtonElement>('.frequency-view-toggle').forEach(button => {
      ...
      btn.classList.remove('btn-light');
      btn.classList.add('active', 'btn-primary');
      frequencyListViewMode = (btn.dataset['frequencyView'] as 'preview' | 'all' | 'issues') ?? 'preview';
      renderFrequencyOverview(getActiveFrequencyStores(currentGlobalFilteredStores).map(getFrequencyMetric));
    });
  });
}
...
showLoadingOverlay('Veriler işleniyor, lütfen bekleyin...');
  try {
    ...
    await Promise.all([loadMasterData(), loadFrequencyData()]);
