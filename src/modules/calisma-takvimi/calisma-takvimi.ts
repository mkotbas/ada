...
export async function initializeCalismaTakvimiModule(pb: PocketBase): Promise<void> {
  ...
  async function saveManualAuditData(): Promise<void> {
    manualAuditData = parseManualAuditMap(manualAuditData);
  ...
  function promptManualCount(month: number, day: number): number | null {
    ...
    const promptMessage = `Bu gün için manuel denetim adedini girin (0-${MAX_MANUAL_AUDIT_COUNT}). Silmek için 0 yazın.`;
    ...
    if (!Number.isFinite(parsedValue) || parsedValue < 0 || parsedValue > MAX_MANUAL_AUDIT_COUNT) {
      notify.warning(`Lütfen 0 ile ${MAX_MANUAL_AUDIT_COUNT} arasında geçerli bir denetim adedi girin.`);
      return null;
    }
  ...
  function renderCalendar(): void {
    ...
    for (let month = 0; month < 12; month += 1) {
      ...
      for (let day = 1; day <= totalDays; day += 1) {
        ...
        const badgeCount = month === currentMonth ? remainingCount : plannedCount;
        ...
        if (dayOfWeek !== 0) {
          ...
          if (isLeave) {
            ...
            appendPlanBadge(dayEl, badgeCount);
          }
        }
