...
import {
  addProductAlternativeRow,
  ensureAlternativeContainer,
  getExistingStylingMainCategoryNames,
  isAlternativeHeader,
  mergeAlternativeProducts,
  normalizeCategoryKey,
  normalizeExcelHeader,
  normalizeAlternativeProduct,
  normalizeProductCode,
  normalizeQuantityValue,
  resolveMappedMainCategoryName,
  type ProductLike,
} from "./soru-yoneticisi-utils";
...
const fallbackFideQuestions = [
  {
    id: 0,
    type: "standard",
    title: "HATA: Sorular buluttan yuklenemedi.",
  },
];
...
async function loadInitialData() {
  ...
  try {
    ...
    (fideQuestions = Array.isArray(cloudData.questions)
        ? cloudData.questions.map(({ answerType: _answerType, ...question }) => question)
        : []),
...
async function applyReportUpdates(updateFunction, options: { message?: string; userMessage?: string; manageOverlay?: boolean } = {}) {
  ...
  try {
    ...
    const allReports = await pbInstance
      .collection("denetim_raporlari")
      .getList(1, 200, { fields: "id,soruDurumlari" })
  ...
  setSafeHtml(
    ...
    `
        ...
        <div class="bulk-add-container bulk-add-styling">
            ...
            <input type="file" class="bulk-styling-input-file" id="${fileInputId}" name="${fileInputId}" accept=".xlsx">
