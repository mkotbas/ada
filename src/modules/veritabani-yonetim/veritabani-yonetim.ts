...
async function analyzeCleanupTargets(pb) {
  const [users, devices, stores, reports, undoneLogs] = await Promise.all([
      fetchAllRecords(pb.collection("users"), { fields: "id" }),
      pb
        .collection(
          "user_devi\
ces",
        )
        .getList(1, 200, { fields: "id,user,updated,created" }).then((result) => result.items),
      fetchAllRecords(pb.collection("bayiler"), { fields: "id" }),
      fetchAllRecords(pb.collection("denetim_raporlari"), {
        sort: "-created",
        fields:
...
ck",
      async () => {
        const users = await pb
            .collection("users")
            .getList(1, 200, { sort: "name" }).then((result) => result.items),
          bodyHtml = `
            <div class="db-form-group">
                <label>\u0130\u015Flem Yap\u0131lacak Kullan\u0131c\u0131y\u0131 Se\xE7in</label>
                <select id="modal-select-user" name="modalSelectUser" class="db-input">
...
<div class="card-option-info"><strong>Verileri Aktar</strong><small>Ba\u015Fka birine ta\u015F\u0131r.</small></div></div>
                ...
                <div id="modal-transfer-select" hidden class="db-form-group">
                    ...
                    <select id="modal-select-target" name="modalSelectTarget" class="db-input"><option value="">-- Hedef Se\xE7in --</option>${users
                      .map(
                        (u) => `<option \
...
odal">Vazge\xE7</button><button id="modal-btn-execute" class="btn-danger btn-sm" disabled>\u0130\u015Flemi Onayla</button>',
        ...
        selectUser.addEventListener("change", async () => {
          ...
          const reports = await pb
              .collection("denetim_raporlari")
              .getList(1, 200, { filter: `user="${userId}"` }).then((result) => result.items),
            bayiler = await fetchAllRecords(pb.collection("bayiler"), {
              filter: `sorumlu\
_kullanici="${userId}"`,
            });
          setSafeHtml(
...
az. Onayl\u0131yor musunuz?",
              ...
              try {
                ...
                if (strategy === "transfer") {
                  const reports = await pb
                    .collection("denetim_raporlari")
                    .getList(1, 200, { filter: `user="${userId}"` })
                    .then((result) => result.items);
                  for (const r of reports)
                    await pb
                      .collection("denetim_raporlari")
                      .update(r.id, { user: targetId });
                  const bayiler = await fetchAllRecords(pb.collection("bayiler"), {
                    filter: `soru\
mlu_kullanici="${userId}"`,
                  ...
                  const bayiler = await pb
                    .collection("bayiler")
                    .getList(1, 200, { filter: `sorumlu_kullanici="${userId}"` })
                    .then((result) => result.items);
                  for (const b of bayiler)
                    await pb
                      .collection("bayiler")
...
button><button id="btn-res-exec" class="btn-primary btn-sm">Geri Y\xFCklemeyi Ba\u015Flat</button>',
    ...
    getEl("btn-action-reset-mappings").addEventListener("click", async () => {
      const list = await pb
        .collection("ayarlar")
        .getList(1, 200, { filter: 'anahtar ~ "excel_mapping_"' })
        .then((result) => result.items);
...
async function clearCollection(pb, col, lbl) {
  const recs = await fetchAllRecords(pb.collection(col), { fields: "id" });
